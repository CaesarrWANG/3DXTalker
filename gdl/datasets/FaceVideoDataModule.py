...
class FaceVideoDataModule(FaceDataModuleBase):
    ...
    def metadata_path(self):
        return os.path.join(self.output_dir, self.video_path.stem, "metadata.pkl")
    ...
    def _unpack_video(self, video_idx, overwrite=False):
        ...
        if not out_folder.exists() or overwrite:
            ...
            # stream.run()
        full_frame_list = sorted(list(out_folder.glob("*.png")))
        truncated_frames = []
        for fi, frame_path in enumerate(full_frame_list):
            try:
                tmp_img = imread(frame_path)
            except (OSError, ValueError) as e:
                print(f"Warning: Frame {frame_path} could not be read and will be removed. Error: {e}")
                truncated_frames.append(frame_path)
                frame_path.unlink() # remove the invalid file
    ...
    def create_reconstruction_video(self, sequence_id, overwrite=False, distance_threshold=0.5,
        ...
        if image_type == "detail":
            outfile = vis_fnames[0].parents[2] / "video.mp4"
        else:
            outfile = vis_fnames[0].parents[2] /( "video_" + image_type + ".mp4")
    ...
    def create_reconstruction_video_with_recognition(self, sequence_id, overwrite=False, distance_threshold=0.5):
        ...
        outfile = vis_fnames[0].parents[2] / baseoutfile
...
def attach_audio_to_reconstruction_video(input_video, input_video_with_audio, output_video=None, overwrite=False):
    ...
    cmd = (
        f"ffmpeg -y -i {str(input_video)} -i {str(input_video_with_audio)} "
        f"-c:v libx264 -pix_fmt yuv420p -c:a aac -map 0:v:0 -map 1:a:0 -shortest {str(output_video)}"
    )
    os.system(cmd)
    return output_video
